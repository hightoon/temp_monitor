...
class Admin extends CI_Controller{
    ...
    public function index(){
        ...
        for($i=0; $i<$allOnlineDevice[0]; $i++){
			if($allOnlineDevice[$i+2] > 0 && $allOnlineDevice[$i+2] < 500){
    ...
    public function realtimeinfo($device_id = FALSE){
        ...
        if ($device_id != FALSE){
            ...
            $this->data['allContorl'] = $this->compair_model->getAllControl($device_id);
    ...
    public function passwdchange(){
		...
		if (FALSE == $this->compair_model->changePasswd($name,$newPasswd)){
            echo 0;
            return;
		}else{
            echo 1;
