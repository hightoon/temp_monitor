...
class Chengdu_model extends CI_Model{
    ...
    public function __construct(){
        ...
        $config['database'] = "ROBOS_CHENGDU_DATA";
        ...
        $config['database'] = "ROBOS_CHENGDU_WEB";        
    ...
    public function getHistoryData($device_id,$info_id,$day=FALSE){
        ...
        $retarray = $ret->result_array();
        
        /*for($i=1;$i<count($retarray);$i++){
            
            if($this->calltimediff($retarray[$i]['update_time'],$retarray[$i-1]['update_time']) > 600){
                
                foreach($retarray[$i] as $key=>$value){
                ...
                foreach($retarray[$i-1] as $key=>$value){
