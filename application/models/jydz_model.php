...
class Jydz_model extends CI_Model{
    ...
    public function __construct(){
        ...
        $config['database'] = "ROBOS_BOLAITE_DATA";
        ...
        $config['database'] = "ROBOS_JYDZ_WEB";        
    ...
    public function getAllMaintain($device_id = FALSE){
        ...
        $sql = "select device_id from all_device_info group by device_id";
        $tmp = $this->dbWeb->query($sql);
    ...
    public function energyAnalysis($day,$device_id){
        ...
        $this->dbData->select()->from('history_table_2')->where(array('update_time > '=>$day['start'],'update_time < '=>$day['end'],'device_id'=>$device_id));
        ...
        $this->dbData->order_by("update_time","desc");
            ...
            $this->dbData->select()->from('history_table_2')->where(array('update_time > '=>$day['start'],'update_time < '=>$day['end'],'device_id'=>$device_id))->limit(1000,$offset);
