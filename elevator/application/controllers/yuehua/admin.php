...
class Admin extends CI_Controller{
    ...
    private function getBrowser(){
        if(empty($_SERVER['HTTP_USER_AGENT'])){
            header("Location:http://10.13.71.176/civ/browser.php");
        }
    ...
    public function index(){
        ...
        $url_encrypt = substr($url_encrypt,strlen("/civ/index.php/".$this->data['admin_dir'].'/admin/index/'));
        ...
        $this->data['allDevice']= $this->yuehua_model->getAllDevice();
        ...
        $allOnlineDevice = $this->yuehua_model->getAllOnlineDevice();/**/
        $this->data['onlineDeviceNum'] = 0;
        for($i=0; $i<$allOnlineDevice[0]; $i++){
			if($allOnlineDevice[$i+2] > 5000 && $allOnlineDevice[$i+2] < 10000){
        ...
        $this->data['allAlarm'] = array();//$this->yuehua_model->getAllAlarm();/**/
        $this->data['alarmNum'] = count($this->data['allAlarm']);
        
        $this->data['allRemark'] = $this->yuehua_model->getAllRemark();/**/
        $this->data['remarkNum'] = count($this->data['allRemark']);
        
        $this->data['allMaintain'] = $this->yuehua_model->getAllMaintain();/**/
    ...
    public function setslist(){
        ...
        foreach($this->data['allDevice'] as $device){
            ...
            $this->data['allWarning']= $this->yuehua_model->getAllAlarm($device['device_id']);
            //$this->data['allMaintain']= $this->yuehua_model->getAllMaintain();  
        }
    ...
    public function basicinfo($device_id=FALSE){
        ...
        if($device_id!=FALSE){
            ...
            $this->data['maintain'] = $this->yuehua_model->getAllMaintain($device_id);
            
            $this->data['device_info'] = $this->yuehua_model->getAllDevice($device_id);
    ...
    public function realtimeinfo($device_id = FALSE){
        ...
        if($this->session->userdata('admin_level') == 99){
            ...
            if($device_id != FALSE){
                $this->data['meters'] = $this->user_yuehua_model->get_robometers($device_id);
                $this->data['device_id'] = $device_id;
            }
        ...
        if ($device_id != FALSE){
			...
			$deviceInfo = $this->yuehua_model->getAllDevice($device_id);

            $this->data['deviceSet'] = $this->yuehua_model->getDeviceSet($deviceInfo['device_type'],$deviceInfo['user_id']);
			
            $isOnline = $this->yuehua_model->getDeviceOnlineStatus($device_id - 8600000);
            ...
            if(0 ==  $isOnline){
                ...
                $this->data['device_id'] = $device_id;
                /*$onlineEqp = $this->yuehua_model->get_allEqp_online();*/
    ...
    public function historyinfo($device_id = FALSE){
        ...
        if ($device_id != FALSE){
            ...
            $this->data['deviceSet'] = $this->yuehua_model->getDeviceSet($deviceInfo['device_type'],$deviceInfo['user_id']);
			$num = 500;
			$this->data['allHistoryInfo'] = $this->yuehua_model->getAllHistoryInfo($device_id,$num);
            $this->data['device_id'] = $device_id;
        }
    ...
    public function infoset(){
		...
		$this->data['deviceSet'] = $this->yuehua_model->getDeviceSet();
    ...
    public function passwdchange(){
		...
		if (FALSE == $this->yuehua_model->changePasswd($name,$newPasswd)){
            echo 0;
            return;
		}else{
            echo 1;
    ...
    public function add_user(){
        ...
        $this->user_yuehua_model->add_user($info);
        echo 0;
        return;
    }
    ...
    public function edit_user(){
        ...
        $this->user_yuehua_model->edit_user($info,$_POST['edituser_name']);
        echo 0;
        return;
    }
    ...
    public function del_user(){
        ...
        $this->user_yuehua_model->del_user($name);
        echo 0;
        return;
    }
}
