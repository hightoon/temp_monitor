...
function addEqp(obj){
	...
	$.ajax({
		type: "POST",
		url: base_url+"index.php/"+admin_dir+"/ajax_setslist/addEqp/",
...
function modalShowDel(obj){
	...
	$('#modalDelYes').mousedown(function(e){
		if(1 == e.which){
			$.ajax({
				type: "POST",
				url: base_url+"index.php/"+admin_dir+"/ajax_setslist/delDevice/",
...
function modalShowEdit(obj){
	var tds=$(obj).parent().parent().find('td');
	var device_id = tds.eq(0).text();
	$('#edit_device_id').text(tds.eq(0).text());
	$('#edit_type').text(tds.eq(1).text());
	...
	$('#editmodalDelYes').mousedown(function(e){
		if(1 == e.which){
			$.ajax({
			   type: "POST",
			   url: base_url+"index.php/"+admin_dir+"/ajax_setslist/editDevice/",
			   dataType:'json',
			   data: {
				0	: device_id,
				1	: $('#edit_type').val(),
				2	: $('#edit_function').val(),
				3	: $('#edit_controller').val(),
				4	: $('#edit_dealer').val(),
				5	: $('#edit_factory').val()
			   },
			   success:function(msg){
...
function map_init(){
	...
	map.centerAndZoom(new BMap.Point(118.9188,35.9422), 5);
...
$(".maintain-remark").click(function(e){
	...
	if(e.currentTarget.parentNode.children[1].innerText == "修改"){
		...
		$.ajax({
			type: "POST",
			url: base_url+"index.php/"+admin_dir+"/ajax_setslist/saveMaintainRemark/",
			dataType    : 'json',
			data: {
				device_id	: id,
				item	: maintain_item,
