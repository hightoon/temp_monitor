...
$(document).ready(function(){
	...
	function getRealtimeData(realtime) {
		...
		if(realtime){
			...
			time_temp0 = myDate.toLocaleTimeString();//data_ajax['update_time'];
			...
			if(true == new_info){
				for(var item in meter_info){
					...
					data_raw[item].push((Math.random()*(100-1)+01).toFixed(2));
				}
			}
		...
		if(document.getElementById('device_id').innerHTML != " - "){
			...
			for(meter in meter_info){
				if(meterlist[meter_info[meter]] != undefined){
					if(meterlist[meter_info[meter]][6] == 1 && meterlist[meter_info[meter]][5] == 1 && (!$("#"+meter_info[meter]+"_flot").parent().is(":hidden"))){
