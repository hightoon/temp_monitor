...
$(function(){
	$("#passwd_new0,#passwd_new1").focus(function(){
		...
		$.ajax({
			type:"POST",
			url:base_url+"index.php/"+admin_dir+"/ajax_userset/passwdcheck/",
...
function  userset0_submit(){
	...
	$.ajax({
		type:"POST",
		url:base_url+'index.php/'+admin_dir+'/admin/index/'+$("#passwd_ch_button").val(),
...
$('#logo_update').click(function(e) {
	...
	e.preventDefault();
	$.ajaxFileUpload({
		url         :base_url+'index.php/'+admin_dir+'/admin/index/'+$("#logo_update").val(),
		secureuri      :false,
...
$("#logo_change").click(function(e){
	$.ajax({
		type:"POST",
		url:base_url+'index.php/'+admin_dir+'/admin/index/'+$("#logo_change").val(),
		success:function(msg){
			if(msg==0){
				/*$("#myModal_type").attr({style:"color:#fff;background:#3c2"});
				$("#myModal_Label").html("成功");
