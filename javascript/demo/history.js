...
$(document).ready(function(){
    ...
    for(var item in meterlist){
        ...
        if(flag == 0 || 1 != meterlist[item][6]){
			flag = 1;
            continue;
        }
        var infoitem = meterlist[item][0]+"info";
        ...
        options['yaxis']['tickFormatter'] = function suffixFormatter(val, axis) {return val+meterlist[item][2];};
        overview[meterlist[item][0]] = $.plot("#overview-"+meterlist[item][0], [d0], {
            series: {
                lines: {
                    show: true,
                    lineWidth: 1,
        ...
        plot_lines[meterlist[item][0]] = $.plot("#"+meterlist[item][0]+"-line-chart", [d0], options);
    }
