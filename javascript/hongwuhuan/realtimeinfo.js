...
$(document).ready(function(){
	...
	function update() {	
		...
		var meters="";
		for (var item in meter_info[0]){
        
            if(0 == data_flot[item].length){
                continue;
            }
            if(undefined != $("#"+meter_info[0][item]+"_status_value").html()){
                len=data_raw[item].length;
                if($("#"+meter_info[0][item]+"_status_value").attr("class") == "On/Off"){
                    if(data_raw[item][len-1] == 0){
                        $("#"+meter_info[0][item]+"_status_value").html("Off");
                    }else{
                        $("#"+meter_info[0][item]+"_status_value").html("On");
                    }
                }
                else{
                    $("#"+meter_info[0][item]+"_status_value").html(data_raw[item][len-1]);
                }
            } 
			
			if(undefined == line_plot[meter_info[0][item]]){
				continue;
			}
			var len=data_raw[item].length;
		
			var pie_data=[];
			pie_data[1] = 100*(data_flot[item][len-1][1]/(meterlist[meter_info[0][item]][3] - meterlist[meter_info[0][item]][4]));
			pie_data[1] = pie_data[1].toFixed(1);
			pie_data[0] = 100 - pie_data[1];
			
			line_plot[meter_info[0][item]].setData([data_flot[item]]);
			line_plot[meter_info[0][item]].draw();
			if (data_flot[item][len-1][1] == undefined){
				$("#"+meter_info[0][item]).html(" - ");
				$("#"+meter_info[0][item]+"_img").css({width:"50%"});
			}else{
				$("#"+meter_info[0][item]).html(data_flot[item][len-1][1]);
				$("#"+meter_info[0][item]+"_img").css({width:pie_data[1]+"%"});
			}
	...
	function cycle(){
		if(document.getElementById('device_id').innerHTML != " - "){
			update();
			for(meter in meter_info[0]){
				if(meterlist[meter_info[0][meter]] != undefined){
					if(meterlist[meter_info[0][meter]][6] == 1 && meterlist[meter_info[0][meter]][5] == 1 ){
						options_chart['yaxis']['min'] = meterlist[meter_info[0][meter]][4];
						options_chart['yaxis']['max'] = meterlist[meter_info[0][meter]][3];
						line_plot[meter_info[0][meter]] = $.plot("#"+meter_info[0][meter]+"_flot", [data_flot[meter]], options_chart);
					}
				}
			}
