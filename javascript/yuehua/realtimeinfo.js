...
$(document).ready(function(){
    ...
    if("8605010" == document.getElementById('device_id').innerHTML || "8605001" == document.getElementById('device_id').innerHTML){
        meter_info = meter_info_temp;
    }else{
        meter_info = meter_info_base;
    }
	...
	$(".meter").click(function(e){
		...
		if(e.target.className == "btn btn-danger btn-sm meter"){
			...
			meterid = "#"+item_name+"_meter";
			$(meterid).fadeIn(1000);
			/*for(var item in meterlist){
				if(meterlist[item][1] == name){
					meterid = "#"+meterlist[item][0]+"_meter";
					meterlist[item][6] = 1;
					$(meterid).fadeIn(1000);
					return;
				}
			...
			$.ajax({
				...
				success: function(msg){
					...
					for(var i=0;i<data0.length && data0[i]!="";i++){
                        ...
                        key = data1[0];
                        var value=data1[1];
                        if(document.getElementById('device_id').innerHTML == 8605001 && key == 24){
                            value = value*10;
